...
import { Plus, Trash2, Edit3, BarChart3, Upload, Loader2, Sparkles, CheckCircle, X, ImagePlus, FileImage, Play, Headphones, CloudUpload, User, Calendar, Download, Users, ShieldCheck, Mail, AlertCircle } from 'lucide-react';
import { db } from '../services/db';
import { geminiService } from '../services/gemini';
import { Media, Category, Admin } from '../types';
...
const AdminDashboard: React.FC<AdminDashboardProps> = ({ onPlay }) => {
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 space-y-8">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          ...
          <p className="text-slate-500 mt-1 uppercase text-xs font-bold tracking-widest">Winners Media Management Portal</p>
        </div>
                      ...
                      <td className="px-6 py-5 text-right">
                        <div className="flex items-center justify-end space-x-1">
                          <button 
                            ...
                            className="p-2.5 text-slate-400 hover:text-red-700 hover:bg-red-50 rounded-xl transition-all active:scale-90"
                            title="Edit message"
                          >
                          ...
                          <button 
                            ...
                            className="p-2.5 text-slate-400 hover:text-red-900 hover:bg-red-50 rounded-xl transition-all active:scale-90"
              ...
              <tbody className="divide-y divide-slate-100/50">
                {adminList.map((admin) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-5">
                      <span className="inline-flex items-center px-3.5 py-1.5 rounded-full text-[10px] font-extrabold bg-amber-50 text-amber-800 border border-amber-100 uppercase tracking-widest shadow-sm">
                        <ShieldCheck className="h-3.5 w-3.5 mr-2" /> Full Access
                      </span>
                    </td>
                    <td className="px-6 py-5 text-right">
                      {adminList.length > 1 && (
                        <button 
                          ...
                          className="p-2.5 text-slate-400 hover:text-red-700 hover:bg-red-50 rounded-xl transition-all active:scale-95"
                          title="Revoke access"
                        >
                          <Trash2 className="h-4.5 w-4.5" />
                        </button>
      ...
      {isAdminModalOpen && (
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-red-950/60 backdrop-blur-sm" onClick={() => setIsAdminModalOpen(false)} />
          <div className="relative bg-white w-full max-w-md rounded-3xl shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200">
            <div className="p-6 border-b border-slate-100 flex items-center justify-between bg-red-50">
              <h3 className="text-xl font-bold text-red-900 font-serif">Add Team Member</h3>
              <button onClick={() => setIsAdminModalOpen(false)} className="text-slate-400 hover:text-red-700">
                <X className="h-6 w-6" />
              </button>
            </div>
            <form onSubmit={handleAddAdmin} className="p-8 space-y-4">
              <div className="space-y-1">
                <label className="text-xs font-bold text-slate-500 uppercase ml-1">Team Member Email</label>
                <input
                  type="email"
                  required
                  value={newAdminEmail}
                  ...
                  className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-red-700 outline-none transition-all text-slate-900 font-bold placeholder-slate-400"
                />
              </div>
              <button
                type="submit"
                className="w-full py-4 bg-red-700 text-white font-bold rounded-xl hover:bg-red-800 transition-all shadow-xl shadow-red-100"
              >
                Register Member
              </button>
            </form>
          ...
          <div className="relative bg-white w-full max-w-2xl rounded-3xl shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200">
            <div className="px-8 py-6 border-b border-slate-100 flex items-center justify-between bg-red-50">
              <h3 className="text-xl font-bold text-red-900 font-serif flex items-center">
                {editingId ? 'Modify Message' : 'Publish New Grace'}
              </h3>
            ...
            <form onSubmit={handleSubmit} className="p-8 overflow-y-auto max-h-[70vh] custom-scrollbar">
                ...
                <div className="space-y-4 pt-4 border-t border-slate-100">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    ...
                    <div className="space-y-2">
                      ...
                      <div className={`relative border-2 border-dashed rounded-2xl p-4 transition-all ${showSuccess ? 'border-green-200 bg-green-50' : 'border-red-100 hover:border-red-300 bg-red-50/20'}`}>
                        {isUploadingFile ? (
                          <div className="space-y-3 py-2">
                            <div className="w-full h-2 bg-slate-200 rounded-full overflow-hidden">
                              <div className="h-full bg-red-700 transition-all duration-300" style={{ width: `${uploadProgress}%` }} />
                            </div>
                          </div>
                        ) : showSuccess ? (
                          <div className="flex items-center justify-between py-2 text-green-700 font-bold text-xs uppercase">
                          ...
                          <label className="flex flex-col items-center justify-center cursor-pointer space-y-2 py-4">
                            <CloudUpload className="h-6 w-6 text-red-700/60" />
                            <span className="text-[10px] font-bold text-red-800 uppercase tracking-widest">Select MP3</span>
                    ...
                    <div className="space-y-2">
                      ...
                      <div className={`relative border-2 border-dashed rounded-2xl p-4 transition-all ${formData.thumbnailUrl ? 'border-amber-200 bg-amber-50/30' : 'border-red-100 hover:border-red-300 bg-red-50/20'}`}>
                        {formData.thumbnailUrl ? (
                          ...
                          <label className="flex flex-col items-center justify-center cursor-pointer space-y-2 py-4">
                            <FileImage className="h-6 w-6 text-red-700/60" />
              ...
              <div className="mt-10 flex items-center space-x-4">
                <button
                  ...
                  className="flex-grow py-4 bg-red-700 text-white font-bold rounded-2xl hover:bg-red-800 transition-all shadow-xl shadow-red-100 disabled:opacity-50"
                >
                  {editingId ? 'Save Record Changes' : 'Publish to Library'}
                </button>
                <button
                  type="button"
                  onClick={closeModal}
